...
* **Hue wheel**: the mode color is stored on a coarse ~12-step wheel
  ...
  through unchanged. `qk80.py` pre-compensates by rounding the hue to the
...
/ `convert_video_ext`); the pure-Python encoder in `qk80.py` produces
...
to the output size. Verified byte-for-byte in `qk80.py` (`encode_slider`,
`album_to_slider`).

### QK80 MK2 import paths (Screen tab)

| UI menu                | Format   | `qk80.py` command                          |
|------------------------|----------|--------------------------------------------|
| Video → Theme (default)| ANIT     | `qk80.py video anim.gif`                   |
| Video → Custom Animation| ANIM    | `qk80.py video anim.gif --variant custom`  |
| Image → Theme (default)| ABKT     | `qk80.py image photo.png`                  |
| Image → Custom Image   | ABKG     | `qk80.py image photo.png --variant custom` |
| Slider → Custom Slider (default)| ANPS | `qk80.py slider 1.png 2.png ...`      |
| Slider → Theme         | ANPT     | `qk80.py slider ... --format ANPT`         |
...
**`qk80.py` controls two independent parts of the matrix:**
...
1. **Mode settings** — color, brightness and the active effect are the
   ...
   `qk80.py matrix color cyan`, `qk80.py matrix brightness 50`,
   `qk80.py matrix effect raindrop`, `qk80.py matrix get`.
   `matrix color` (and the bare-colorname shorthand `qk80.py matrix cyan`)
...
2. **Custom grid** — the 7×7 grid itself, uploaded as a frame over
   ...
   (`qk80.py matrix custom cyan`), manual patterns
...
* **Format verification**: `kbres.wasm` from the repo is byte-identical to the
  ...
  in Node produced ABKG/ANIM files that `qk80.py` reproduces **byte-for-byte**,
